...
int smpexpr (int inherited_type)
{
   ...
   F_Entry:
	...
	case ID:
		...
		match(ID);
		if(lookahead == ASGN && !isRValue) {		// ASGN = ":="
			/* located variable is LVALUE */
			lvalue_seen = 1;
			ltype = syntype;
			match(ASGN);
			isRValue = 1;  // Lock assignment token until rvalue expr ends
			rtype = expr(ltype);
			isRValue = 0; // Unlock assignment token
		...
		else if(lookahead == ASGN && isRValue) {
			match(ID); //force error
